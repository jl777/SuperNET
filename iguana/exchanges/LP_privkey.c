...
void verusblocks(struct iguana_info *coin,char *coinaddr)
{
    ...
    if ( bits256_nonz(hash) != 0 )
    {
        ...
        while ( (blockjson= LP_blockjson(&height,coin->symbol,hashstr,0)) != 0 )
        {
            ...
            {
                printf("num.%d PoW %.2f%% %.8f %d v %d PoS %.2f%% %.8f -> %.8f supply %.8f PoW %.1f%% PoS %.1f%% both %.1f%% RTu3 %.8f\n",num,100.*(double)numpow/num,powsum,npow,npos,100.*(double)numpos/num,possum,powsum+possum,supply,100.*powsum/supply,100.*possum/supply,100.*(powsum+possum)/supply,RTu3sum);
            }
        }
    }
    if ( num > 0 )
    {
        ...
        printf("mytimelocked\n");
        printf("num.%d PoW %.2f%% %.8f %d v %d PoS %.2f%% %.8f -> %.8f supply %.8f PoW %.1f%% PoS %.1f%% both %.1f%% RTu3sum %.8f\n",num,100.*(double)numpow/num,powsum,npow,npos,100.*(double)numpos/num,possum,powsum+possum,supply,100.*powsum/supply,100.*possum/supply,100.*(powsum+possum)/supply,RTu3sum);
    }
}
