                ...
                if ( np->datalen == (size - sizeof(*np)) )
                {
                    ...
                    if ( dp != 0 && crc32 == np->crc32 )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                char str[65]; printf("%s RECV ht.%d ch.%08x (%d) crc32.%08x:%08x datalen.%d:%d firstz.%d i.%d senderind.%d\n",bits256_str(str,np->srchash),np->height,np->channel,size,np->crc32,crc32,np->datalen,(int32_t)(size - sizeof(*np)),firstz,i,np->senderind);
