...
void iguana_appletests(struct supernet_info *myinfo)
{
    ...
    if ( 1 )
    {
       ...
       getchar();**/
        ...
        {
            free(str);
            if ( 1 && (str= SuperNET_JSON(myinfo,cJSON_Parse("{\"portp2p\":8333,\"RELAY\":1,\"VALIDATE\":1,\"agent\":\"iguana\",\"method\":\"addcoin\",\"startpend\":1,\"endpend\":1,\"services\":128,\"maxpeers\":8,\"newcoin\":\"BTC\",\"active\":0,\"numhelpers\":1,\"poll\":100}"),0,myinfo->rpcport)) != 0 )
            {
                free(str);
