...
pub fn de_none_if_empty<'de, D: Deserializer<'de>> (des: D) -> Result<Option<String>, D::Error> {
    ...
    impl<'de> serde::de::Visitor<'de> for Visitor {
        ...
        fn visit_none<E> (self) -> Result<Option<String>, E> where E: serde::de::Error {Ok (None)}
        fn visit_unit<E> (self) -> Result<Option<String>, E> where E: serde::de::Error {Ok (None)}
        fn visit_str<E> (self, sv: &str) -> Result<Option<String>, E> where E: serde::de::Error {
            if sv.is_empty() {
                Ok (None)
            } else {
                Ok (Some (sv.into()))
