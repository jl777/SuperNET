...
fn test_bincode() {
    ...
    log!((std::mem::size_of_val(&maker)));
    let serialized = bincode::serialize(&maker).unwrap();
    log!([serialized]);
    log!("Bincode len " (serialized.len()));

    use rmp_serde::{Deserializer, Serializer};
    use serde::{Deserialize, Serialize};

    let mut buf = Vec::new();
    maker.serialize(&mut Serializer::new(&mut buf)).unwrap();
    log!([buf]);
    log!("MsgPack len " (buf.len()));

    let json = json::to_vec(&maker).unwrap();
    log!("JSON len " (json.len()));

    let cbor = serde_cbor::to_vec(&maker).unwrap();
    log!([cbor]);
    log!("CBOR len " (cbor.len()));


    log!("NUMBER >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
    let num = BigRational::new(std::u32::MAX.into(), std::u128::MAX.into());

    let serialized = bincode::serialize(&num).unwrap();
    log!([serialized]);
    log!("Bincode len " (serialized.len()));

    let mut buf = Vec::new();
    num.serialize(&mut Serializer::new(&mut buf)).unwrap();
    log!([buf]);
    log!("MsgPack len " (buf.len()));

    let json = json::to_vec(&num).unwrap();
    log!("JSON len " (json.len()));

    let cbor = serde_cbor::to_vec(&num).unwrap();
    log!([cbor]);
    log!("CBOR len " (cbor.len()));
    ...
    let old = OldMsg { price: num.clone().into() };
    let cbor = serde_cbor::to_vec(&old).unwrap();
    log!([cbor]);
    log!("CBOR len " (cbor.len()));

    let mut new: NewMsg = serde_cbor::from_slice(&cbor).unwrap();
    log!([new]);
    assert_eq!(new.price, old.price);

    new.volume = Some(num.clone().into());

    let new_cbor = serde_cbor::to_vec(&new).unwrap();
    let old: OldMsg = serde_cbor::from_slice(&new_cbor).unwrap();
    log!([old]);
    assert_eq!(new.price, old.price);

    let msg_pack = rmp_serde::to_vec(&old).unwrap();

    let new_msg_pack: NewMsg = rmp_serde::from_read_ref(&msg_pack).unwrap();
    assert_eq!(old.price, new_msg_pack.price);
}
