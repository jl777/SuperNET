...
pub fn clientnode(seednode_ip: &str, seednode_port: u16) {
    // Create a random PeerId
    let local_key = identity::Keypair::generate_ed25519();
    let local_peer_id = PeerId::from(local_key.public());
    println!("Local peer id: {:?}", local_peer_id);
    ...
    let to_dial = format!("/ip4/{}/tcp/{}", seednode_ip, seednode_port).parse().unwrap();
    match libp2p::Swarm::dial_addr(&mut swarm, to_dial) {
        Ok(_) => println!("Dialed {}", seednode_ip),
        Err(e) => println!("Dial {:?} failed: {:?}", seednode_ip, e),
    }
    let mut last_msg = 0;
    let mut msg_id = 0;
    task::block_on(future::poll_fn(move |cx: &mut Context| {
        loop {
            ...
            if now - last_msg > 5000 {
                swarm.publish(&topic, format!("Hello {}", msg_id));
                last_msg = now;
                msg_id += 1;
            }
