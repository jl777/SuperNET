...
*/
pub unsafe fn lp_bob_loop(swap: *mut lp::basilisk_swap) {
    let mut err = 0;
    ...
    */
    if !swap.is_null() && err == 0 {
        if lp::LP_waitsend(b"pubkeys\x00".as_ptr() as *mut c_char,120,(*swap).N.pair,swap,data.as_mut_ptr(),maxlen,Some(lp::LP_pubkeys_verify),Some(lp::LP_pubkeys_data)) < 0 {
            err = -2000;
            log!("error waitsend pubkeys");
        } else if lp::LP_waitsend(b"choosei\x00".as_ptr() as *mut c_char,lp::LP_SWAPSTEP_TIMEOUT as i32,(*swap).N.pair,swap,data.as_mut_ptr(),maxlen,Some(lp::LP_choosei_verify),Some(lp::LP_choosei_data)) < 0 {
            err = -2001;
            log!("error waitsend choosei");
        } else if lp::LP_waitsend(b"mostprivs\x00".as_ptr() as *mut c_char,lp::LP_SWAPSTEP_TIMEOUT as i32,(*swap).N.pair,swap,data.as_mut_ptr(),maxlen,Some(lp::LP_mostprivs_verify),Some(lp::LP_mostprivs_data)) < 0 {
            err = -2002;
            ...
            */
            if err == 0 {
                if lp::LP_swapdata_rawtxsend((*swap).N.pair,swap,0x200,data.as_mut_ptr(),maxlen,&mut (*swap).bobdeposit,0x100,0) == 0 {
                    err = -2005;
            ...
            if err == 0 {
                ...
                if lp::basilisk_bobscripts_set(swap,0,1) < 0 {
                    ...
                    lp::LP_unavailableset((*swap).bobpayment.utxotxid,(*swap).bobpayment.utxovout,(now_ms() / 1000) as u32 + 60,(*swap).I.otherhash);
                    lp::LP_swap_critical = (now_ms() / 1000) as u32;
                    if lp::LP_swapdata_rawtxsend((*swap).N.pair,swap,0x8000,data.as_mut_ptr(),maxlen,&mut (*swap).bobpayment,0x4000,0) == 0 {
                        err = -2008;
...
pub unsafe fn lp_alice_loop(swap: *mut lp::basilisk_swap) {
    ...
    */
    if !swap.is_null() && err == 0 {
        ...
        if lp::LP_sendwait(b"pubkeys\x00".as_ptr() as *mut c_char, 120, (*swap).N.pair, swap, data.as_mut_ptr(), maxlen, Some(lp::LP_pubkeys_verify), Some(lp::LP_pubkeys_data)) < 0 {
            err = -1000;
            log!("error LP_sendwait pubkeys");
        } else if lp::LP_sendwait(b"choosei\x00".as_ptr() as *mut c_char, lp::LP_SWAPSTEP_TIMEOUT as i32, (*swap).N.pair, swap, data.as_mut_ptr(), maxlen, Some(lp::LP_choosei_verify), Some(lp::LP_choosei_data)) < 0 {
            err = -1001;
            log!("error LP_sendwait choosei");
        } else if lp::LP_sendwait(b"mostprivs\x00".as_ptr() as *mut c_char, lp::LP_SWAPSTEP_TIMEOUT as i32, (*swap).N.pair, swap, data.as_mut_ptr(), maxlen, Some(lp::LP_mostprivs_verify), Some(lp::LP_mostprivs_data)) < 0 {
            err = -1002;
            ...
            if lp::LP_waitfor((*swap).N.pair, swap, bobwaittimeout, Some(lp::LP_verify_bobdeposit)) < 0 {
                err = -1005;
                ...
                //(*swap).sentflag = 1;
                lp::LP_swap_critical = (now_ms() / 1000) as u32;
                if lp::LP_waitfor((*swap).N.pair, swap, bobwaittimeout, Some(lp::LP_verify_bobpayment)) < 0 {
                    err = -1007;
