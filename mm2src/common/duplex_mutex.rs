...
impl<T> Impl<T> {
    ...
    fn unlock (&self) -> Result<(), String> {
        if self.locked.compare_exchange (1, 0, Ordering::Relaxed, Ordering::Relaxed) .is_ok() {return Ok(())}
        ERR! ("Not locked")
    }
...
impl<T> Clone for DuplexMutex<T> {
    fn clone (&self) -> DuplexMutex<T> {
        DuplexMutex {pimpl: self.pimpl.clone()}
}   }

impl<T> DuplexMutex<T> {
    pub fn new (v: T) -> DuplexMutex<T> {
        DuplexMutex {pimpl: Arc::new (Impl {locked: Atomic::new (0), data: UnsafeCell::new (v)})}
}   }
